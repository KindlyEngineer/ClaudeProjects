...
### Horizon 2 — "The Living War" ✅ COMPLETE (all six pieces shipped)
| Piece | Ruling |
|---|---|
...
| **Electronic warfare** (D15 — pulled forward) ✅ | Jammers (blind sensor radii) and DECOYS that inject false sightings into the enemy's belief map — attacking the game's defining substrate directly. **Shipped** (H2-EW): `ew_vehicle` with jam umbrella (burn-through, air flies over) + phantom decoys (believed, never fireable, blown on scout). AI decoy use deferred to the persistent-enemy slice. |
| Persistent enemy force ✅ | The opposing formation carries ITS losses/damage across the operation; scouting their remaining strength becomes operational intel. **Shipped** (H2-ENEMY): per-type-max formation, same-type slot filling, half-repair refits, fog-honest confirmed-kill intel in the Interlude. |
| Service records & the wall ✅ | Kill tallies, battle honors, the fallen remembered by name — permanence made visible from the event stream. **Shipped** (H2-RECORDS): per-name battles/kills/resupplies + `fellAt`, Interlude tallies, the wall at operation's end. |
| Generated/branching operations ✅ | Seeded campaign generation on the map generator + force budgets; handcrafted ops branch on outcomes. **Shipped** (H2-OPGEN): `genop` campaigns regenerate deterministically from (defId, seed); escalation presets; full trust/enemy/records machinery for free. Branching handcrafted ops remain open (H3 candidate). |
